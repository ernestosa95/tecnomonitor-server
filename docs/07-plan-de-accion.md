...
|---|---|---|---|
| 2.1 | Diseñar y agregar autenticación al endpoint de ingesta: API key por hospital (header, validada contra `hospitales_metadata` o tabla nueva) + rate limiting por IP/hospital_id. **Requiere actualizar el agente en cada hospital para que mande la key** — coordinar el rollout (agentes viejos no deberían quedar bloqueados de un día para el otro). | `main.py:82` (`/v1/hospital-status`) | [S2](04-seguridad.md#s2) — HIGH, el hallazgo de seguridad más importante después de las credenciales filtradas |
...
| 2.4 | Normalizar `app_name` a minúsculas al escribir en `software_monitoring` (ya se hace para el diccionario de logs) y reemplazar los loops por-hospital en `verificar_estado_software`/`_verificar_mirth` por una sola query con `WHERE hospital_id IN (...)`. | `dashboard_app/alerts_engine.py:820-1141` | [P3](05-performance.md#p3) |
